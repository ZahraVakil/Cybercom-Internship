...
qouteBtn = document.querySelector('.btn'),
...
function saveQuoteToLocalStorage(quote) {
  ...
  function postQuoteToFavoritesAPI(quote) {
    fetch("https://jsonplaceholder.typicode.com/posts", {
      ...
      body: JSON.stringify({
        title: quote.author, // You can use the author as the title
        body: quote.content // Use the quote content as the body
      })
    })
    .then(response => {
      if (!response.ok) {
        throw new Error('Failed to save quote as favorite');
      }
  ...
  starBtn.addEventListener('click', () => {
    const quote = {
      content: qouteText.innerText,
      author: authorName.innerText
    };
    
    saveQuoteToLocalStorage(quote);
    postQuoteToFavoritesAPI(quote);
  });
